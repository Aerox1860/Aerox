...
function CompactMatchRow({ match, tick, seed }) {
  ...
  //   - "RAIN" if admin flagged rain_delay
  //   - "LIVE" if match_time is null OR match_time has already passed
  //   - "TODAY" otherwise (upcoming — user asked to hide LIVE until start-time)
  const now = Date.now();
  ...
  return (
    ...
    >
      ...
      <div className="text-[10.5px] leading-tight">
        <span className={`text-[8.5px] font-bold px-1 py-0.5 rounded uppercase tracking-widest ${badge.tone}`}>{badge.label}</span>
