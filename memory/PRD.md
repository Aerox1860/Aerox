...
- **Match status badge + Rain Delay (2026-02-05)**: Row-header status is now dynamic. Backend `featured_matches` gained `rain_delay: bool` (default False) and a new admin endpoint `POST /api/admin/featured/matches/{id}/toggle-rain`. Player UI (both Lobby `CompactMatchRow` and InPlay `MatchTableRow`) computes: `RAIN DELAY` (blue, pulsing) if the flag is set, else `LIVE` (red) if `match_time` is null or already passed, else `TODAY` (amber). The status re-evaluates every 2 s via the existing `tick` so a scheduled match auto-flips to LIVE at start-time without a page refresh. Admin Matches page has a new `Rain`/`Rain On` toggle button beside `Live`.
- **Wallet + Me white redesign (2026-02-05)**: `Wallet.jsx` and `Profile.jsx` rebuilt to match app-wide white theme. Wallet: white balance card with soft cyan halo, gradient green Deposit + gradient red Withdraw CTAs, colourful quick-chip row (Add money / Withdraw / Rank), light transactions list with type-tinted amounts. Profile/Me: white avatar card, 3 tinted stat cards (Bets / Won / Profit-Loss), gold Daily-bonus banner, 2×2 colour-coded nav pills (Wallet cyan · Referrals fuchsia · Rank amber · Support emerald), full-width red Logout, and a light recent-bets list.
- **Deposit Limits — one-shot, admin-only reset (2026-02-05)**: Tightened responsible-gambling posture. `POST /api/limits/set` now returns 403 if a limit already exists (user cannot modify their own cap). `DELETE /api/limits/me` is disabled — returns 403 "Only admin can reset". Admin's `POST /api/admin/limits/{user_id}/reset` unchanged. Player Limits page rebuilt: if limit already set → read-only card (cap + used + remaining + set-date) plus red "Limit is locked → Contact support" panel with no edit controls. If no limit → editor with suggestions and a mandatory "I understand only admin can reset this" confirmation checkbox before the Set button enables. curl-verified: 1st SET 200 → 2nd SET 403 → DELETE 403 → admin reset 200 → SET again 200.
