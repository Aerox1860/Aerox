...
- **Virtual Cricket Phase 1 (Feb 2026)**: `/virtual` route + featured card on `/in-play`. Backend engine (`virtual.py`) runs 3 concurrent simulated T5 matches, ball-by-ball ticks every 2s, ~2 min per match. Markets: `match_winner` (with mid-match cashout), `toss_winner` (locks at toss), `total_runs` (over/under, settles at match end). Dynamic odds recomputed every ball using a chase-probability model + house margin. WebSocket at `/api/virtual/ws/{match_id}` streams live state. Auto-settlement + wallet payout on match end. Tested 13/13 backend + full frontend flow (bet → cashout → settle).
