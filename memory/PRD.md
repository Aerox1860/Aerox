...
- **Virtual Cricket Phase 1 (Feb 2026)**: `/virtual` route + featured card on `/in-play`. Backend engine (`virtual.py`) runs 3 concurrent simulated T20 matches, ball-by-ball ticks every 6s, ~30 min per full match cycle. Match phases: `pre_match` (3 min betting window) → `toss` (25s animated) → `lineup` (60s team walkout) → `innings1` (~12 min) → `break` (90s) → `innings2` → `completed`. Markets: `match_winner` (with mid-match cashout), `toss_winner` (locks when toss phase begins), `total_runs` over/under (settles at match end). Dynamic odds re-computed every ball. WebSocket at `/api/virtual/ws/{match_id}` streams live state. Team pools: International (10 countries) + Domestic (12 Indian states). Rotation: slot 0 domestic, slots 1+2 international. Fixed stake chips: ₹100/₹500/₹1k/₹5k/₹10k. Commentary shows only current + previous over. Tested 13/13 backend.
